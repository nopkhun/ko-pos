...
### The POS says to cancel Beam first, but there is no Cancel button and every other method is blocked

**Symptom:** after choosing Card or PromptPay, the cashier cannot select another payment
method. Cancelling in the Beam Bolt app does not visibly unlock Odoo, and the KO page only
says to wait or cancel even though it offers no way to cancel.

**Cause:** Odoo 19's standard payment screen renders `PaymentScreenPaymentLines`, which
shows terminal states and calls `sendPaymentCancel`. The KO payment template replaced the
whole screen and omitted that component. Odoo therefore correctly kept its global
`paymentTerminalInProgress` safety lock while the Beam adapter's pending Promise was hidden
from the cashier. A second race made app-first cancellation worse: Beam can reject a second
cancel request even though the original Bolt Intent is already final. The old five-second
cooldown also lived only on the first payment line, so a newly-created shared PromptPay line
could bypass it.

**Fix (`ko_pos_beam_bolt` 19.0.3.1.0 / `ko_pos_ui` 19.0.6.3.0):** the KO page now exposes
the terminal status and a red **ยกเลิกรายการ · Cancel** button wired to Odoo's own
`sendPaymentCancel` lifecycle. The Beam adapter reconciles a rejected cancel with a GET of
the same Bolt Intent; only a final cancel, expiry, or failed Charge releases the lock, while
a success or uncertain result stays blocked. The device-ready deadline is stored on the POS
