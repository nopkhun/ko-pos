...
- **Beam terminal-cancellation lifecycle hotfix is deployed in production.**
  ...
  `sendPaymentCancel`, and refuses to leave or switch terminals until the line reaches a
  safe final/retry state. If the customer cancelled in the Bolt app first, the adapter
  re-reads the same Intent before releasing Odoo's `paymentTerminalInProgress` lock. The
  five-second Beam cooldown is shared across Card, PromptPay, and other methods on that POS.
  Static JS/XML/Python checks, a Node lifecycle regression suite, and a disposable Odoo 19
